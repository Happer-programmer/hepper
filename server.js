...
function onChatHandler(target, context, msg, self) {
  ...
  if (commandName === '!social') {
    client.say(target, `socials at www.twitter.com and www.discord.com/uncle`);
  }

  if (commandName === '!game') {
    client.say(target, 'Uncle is playing League of legends for 24/7🍕🍕🍕🍕', 'pizz');
  }

  if (msg.includes("did you know?") || msg.includes("Did you know?")) {
    client.say(target, 'did you know that among us is among us?', 'dead');
  }

  if (msg.includes("hello") || msg.includes("HELLO")) {
    client.say(target, 'welcome to the chat, enjoy your stay :)', 'sp');
  }
